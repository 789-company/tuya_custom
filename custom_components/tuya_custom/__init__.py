"""Support for Tuya Smart devices (Tuya Custom)."""
...
async def async_setup_entry(hass: HomeAssistant, entry: TuyaConfigEntry) -> bool:
    ...
    # ────────────────────────────────────────────────────────────────────────────
    # WORKAROUND ENTRY POINT: schedule your polling fallback here.
    # Extend ``polling_fallback`` to register ``async_track_time_interval`` or
    # any scheduler that periodically calls ``manager.update_device_cache``.
    # ────────────────────────────────────────────────────────────────────────────
    await polling_fallback(hass, entry, manager)
    return True
...
class DeviceListener(SharingDeviceListener):
    ...
    def update_device(
        ...
        # WORKAROUND ENTRY POINT: inject custom DP debugging here. For example,
        # write the payload to a file or forward it to persistent notifications
        # to analyse missing MQTT events.
        dispatcher_send(
            self.hass,
...
class TokenListener(SharingTokenListener):
    ...
    def update_token(self, token_info: dict[str, Any]) -> None:
        ...
        # WORKAROUND ENTRY POINT: trigger an integration-level auto reload when
        # credentials change or when you want to force a refresh from your
        # custom polling logic. Example:
